...
export class RendererViewport {
  ...
  public constructor(createInfo: RendererViewportCreateInfo) {
    this._width = createInfo.width ?? 0 // TODO: update these default values
    this._height = createInfo.height ?? 0
    this._x = createInfo.x ?? 0
    this._y = createInfo.y ?? 0
  ...
  public OnResize(width: number, height: number): void {
    ...
    if (this._canvas !== undefined) {
      if (this._sizeMode === ViewportSizeMode.DYNAMIC) {
        ...
        if (!gameArea) {
          throw new Error('Could not find gameArea element.')
        }
  ...
  private regenerateMatrix(): void {
    if (this._projectionType === ViewportProjectionType.ORTHOGRAPHIC) {
      ...
      this._projection = Matrix4x4.perspective(
        this._fov,
