...
export class MessageBus {
  ...
  public static addSubscription(
    ...
    if (matches.length === 0) {
      ...
      console.warn(
        'Attempting to add a duplicate handler/callback to code: ' +
          code +
      ...
      if (message.priority === MessagePriority.HIGH) {
        if (h.handler !== undefined) {
          ...
          if (h.callback !== undefined) {
            ...
            console.log(
              'There is no hander OR callback for message code: ' +
                message.code,
            )
          }
        }
        ...
        MessageBus._normalMessageQueue.push(
          new MessageQueueNode(message, h.handler, h.callback),
        )
      }
    }
  }
  ...
  public static update(time: number): void {
    ...
    for (let i = 0; i < messageLimit; ++i) {
      ...
      if (node?.handler !== undefined) {
        ...
        console.warn(
          'Unable to process message node because there is no handler or callback: ' +
            node,
        )
      }
    }
