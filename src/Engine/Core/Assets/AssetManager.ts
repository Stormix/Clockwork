...
export class AssetManager {
  ...
  public static loadAsset(assetName: string): void {
    ...
    for (const l of AssetManager._loaders) {
      if (extension && l.supportedExtensions.indexOf(extension) !== -1) {
    ...
    console.warn(
      'Unable to load asset with extension ' +
        extension +
        ' because there is no loader associated with it.',
    )
  }
  ...
  /**
   * Attempts to get an asset with the provided name. If found, it is returned; otherwise, null is returned.
