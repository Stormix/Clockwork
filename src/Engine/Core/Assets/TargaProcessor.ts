...
class TargaHeader {
  ...
  public initialize() {
    ...
    this.isGreyColor =
      this.imageType === TargaType.RLE_GREY || this.imageType === TargaType.GREY
  }
}
  ...
  public static loadToDataUrl(rawData: any): string {
    ...
    if (header.hasColorMap) {
      ...
      palette = new Uint8ClampedArray(
        data.subarray(offset, offset + colorMapSize),
      )
      offset += colorMapSize
    }
    ...
    if (header.hasEncoding) {
      imageDataArray = new Uint8ClampedArray(
        TargaProcessor.decodeRLE(data, offset, pixelSize, pixelTotal),
      )
    } else {
      imageDataArray = new Uint8ClampedArray(
        data.subarray(
    ...
    const origin = (header.flags & TargaOrigin.MASK) >> TargaOrigin.SHIFT
  ...
  private static checkHeader(header: TargaHeader): void {
    ...
    if (header.hasColorMap) {
      ...
      if (header.colorMapType) {
        throw new Error(
          'Targa - Image contains color map for non-indexed type.',
        )
      }
    }
